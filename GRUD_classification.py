...
grud = GRUD(n_steps=48, n_features=37, rnn_hidden_size=128, n_classes=2, epochs=10)

# saits = SAITS(n_steps=48, n_features=37, n_layers=2, d_model=256, d_inner=128, n_head=4, d_k=64, d_v=64, dropout=0.1, epochs=2)
grud.fit(X, y)  # train the model. Here I use the whole dataset as the training set, because ground truth is not visible to the model.
