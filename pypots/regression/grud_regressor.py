...
from pypots.regression.base import BaseNNRegressor
...
class _GRUD_REGRESSOR(nn.Module):
    def __init__(self, n_steps, n_features, rnn_hidden_size, n_classes, device=None, n_output=1, window=49, horizon=1):
        ...
        self.regression_head = nn.Linear(self.rnn_hidden_size, self.n_output)
...
class GRUD_REGRESSOR(BaseNNRegressor):
    ...
    def predict_on_train(self, train_X, train_y, val_X=None, val_y=None):
        train_X, train_y = self.check_input(
            self.n_steps, self.n_features, train_X, train_y
        )
        ...
        training_set = DatasetForGRUD(train_X, train_y)
        training_loader = DataLoader(
            training_set, batch_size=1, shuffle=False
        )

        if val_X is None:
            return self._model_predict(training_loader)
        else:
            val_X, val_y = self.check_input(self.n_steps, self.n_features, val_X, val_y)

            val_set = DatasetForGRUD(val_X, val_y)
            val_loader = DataLoader(val_set, batch_size=1, shuffle=False)
            return self._model_predict(training_loader, val_loader)
